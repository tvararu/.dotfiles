...
- **Broker config**: `sys/t1/mosquitto.conf`; credentials in `~/srv/mosquitto/passwd` (generated on the box with `mosquitto_passwd`, never committed), users `ha`, `t1` and `sol`. The container mounts the passwd file read-only, so add users against the host copy via a one-off container: `docker run --rm -i -v ~/srv/mosquitto/passwd:/pw eclipse-mosquitto:2 mosquitto_passwd -b /pw <user> <pw>`, then restart. Since 2026-08-30 the broker is also on the tailnet as `tcp://100.73.138.96:1883` via `tailscale serve --bg --tcp 1883 tcp://127.0.0.1:1883` — Serve rather than a published-port bind, per *Do not bind published ports to a Tailscale IP* below, and no ufw rule needed per *Tailscale accepts inbound before ufw sees it*
- **Publisher**: `sys/host-metrics.py` (shared with sol since 2026-08-30 — device block from DMI, SATA SMART and ZFS readers are hardware-gated), run by `t1-metrics.service` — **copied** into `/etc/systemd/system/`, deliberately not symlinked (see *The unit must not be symlinked from this repo* below). Edits to the `.py` apply after `systemctl restart t1-metrics`; edits to the unit need re-copying first
